...
const WHATSAPP_NUMBER = process.env.WHATSAPP_NUMBER || '2349065828886';
...
function normalizeCategory(value) {
  return cleanString(value, 50) || 'Fashion';
}
...
app.get('/api/health', async (req, res) => {
  ...
  res.json({
    ok: true,
    service: 'Face of Style Hijab Factory',
    database,
...
app.post('/api/admin/products', requireAdmin, async (req, res) => {
  ...
  const color = cleanString(req.body?.color, 200);
  ...
  const stock = Number.isInteger(stockNumber)
    ? stockNumber
    : 0;
...
app.post('/api/paystack/initialize', async (req, res) => {
  ...
  const name = cleanString(customer.name, 150);
  const email = cleanString(customer.email, 200);
  const phone = cleanString(customer.phone, 40);
  const address = cleanString(customer.address, 1000);
  ...
  try {
    ...
    for (const rawItem of rawItems) {
      const productId = positiveInteger(rawItem.id);
      const quantity = positiveInteger(rawItem.qty);
